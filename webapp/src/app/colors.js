// utility to manage a list of colors
// currently limited to 150 colors

const colors = [[120, 120, 120], [180, 120, 120], [6, 230, 230], [80, 50, 50], [4, 200, 3],
...
function getUnusedColorIndex(colorsUsed) {
    for (let colorIndex = 0; colorIndex < colors.length; colorIndex++)
        if (!colorsUsed.find(value => isColorEqual(value,colors[colorIndex]))) return colorIndex;
    return -1
}

function getUnusedColor(colorsUsed) {
    const colorIndex = getUnusedColorIndex(colorsUsed);
    return colorIndex >= 0 ?  colors[colorIndex] : null;
}

function countUnusedColors(colorsUsed) {
    return colors.length - colorsUsed.length;
}

export { getUnusedColor, countUnusedColors }